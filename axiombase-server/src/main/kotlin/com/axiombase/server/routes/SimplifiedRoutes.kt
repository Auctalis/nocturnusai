 ...
 *   POST /tell    — Tell AxiomBase a fact (wraps /assert/fact)
 *   POST /ask     — Ask AxiomBase a question (wraps /infer)
 *   POST /teach   — Teach AxiomBase a rule (wraps /assert/rule)
 *   POST /forget  — Make AxiomBase forget something (wraps /retract)
 *
 * Memory aliases:
...
fun Route.simplifiedRoutes(dbManager: DatabaseManager) {
    ...
    // TELL — "Tell AxiomBase something it should know"
    post("/tell") {
        try {
    ...
    post("/teach") {
        try {
            ...
            val headAtom = com.axiombase.core.Atom(req.head.predicate, headTerms, truthVal = !req.head.negated, scope = req.head.scope, metadata = req.head.metadata)

            val bodyAtoms = req.body.map { atomReq ->
                val terms = atomReq.args.map { parseTerm(it) }
                com.axiombase.core.Atom(atomReq.predicate, terms, truthVal = !atomReq.negated, scope = atomReq.scope, metadata = atomReq.metadata)
            }
    ...
    // FORGET — "Make AxiomBase forget something (and everything derived from it)"
    post("/forget") {
        try {
            val (db, tenantId) = call.getContext(dbManager)
            ...
            val terms = req.args.map { parseTerm(it) }
            val atom = com.axiombase.core.Atom(req.predicate, terms, scope = req.scope)
...
private fun serializeSimplifiedEvent(event: com.axiombase.memory.KnowledgeEvent): String {
