...
import com.nocturnusai.testing.TestCase
...
class Parser(private val tokens: List<Token>) {
    ...
    private fun parseTest(): Command {
        ...
        val setupActions = mutableListOf<com.nocturnusai.testing.SetupAction>()
        val expectations = mutableListOf<com.nocturnusai.testing.Expectation>()
    ...
    private fun parseSetupAction(): com.nocturnusai.testing.SetupAction {
        ...
        return if (check(TokenType.FORALL)) {
            ...
            com.nocturnusai.testing.SetupAction.AssertRule(rule)
        } else {
            val fact = parseAtom()
            consume(TokenType.SEMICOLON, "Expected ';' after fact in GIVEN")
            com.nocturnusai.testing.SetupAction.AssertFact(fact)
        }
    }

    private fun parseExpectation(): com.nocturnusai.testing.Expectation {
        return when {
            match(TokenType.PROVABLE) -> {
                val goal = parseAtom()
                consume(TokenType.SEMICOLON, "Expected ';' after PROVABLE expectation")
                com.nocturnusai.testing.Expectation.Provable(goal)
            }
            match(TokenType.NOT_PROVABLE) -> {
                val goal = parseAtom()
                consume(TokenType.SEMICOLON, "Expected ';' after NOT_PROVABLE expectation")
                com.nocturnusai.testing.Expectation.NotProvable(goal)
            }
            match(TokenType.COUNT) -> {
                ...
                com.nocturnusai.testing.Expectation.ResultCount(goal, count)
            }
