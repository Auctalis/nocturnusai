"""Async and sync HTTP clients for the AxiomBase server API.
...
logger = logging.getLogger("axiombase")
...
class AxiomBaseClient:
    """Async HTTP client for the AxiomBase inference engine API.

    This client communicates with the AxiomBase server over HTTP, providing
    methods for asserting facts and rules, running inference, managing memory
    lifecycle, and executing Logiql DSL commands.

    Args:
        base_url: The base URL of the AxiomBase server (e.g., ``http://localhost:9300``).
    ...
    async def _request(
        ...
        *,
        ...
        Raises:
            AxiomBaseAPIError: For non-retryable server errors.
            AxiomBaseConnectionError: When the server is unreachable after retries.
            AxiomBaseTimeoutError: When the request times out after retries.
        """
        ...
        for attempt in range(self._max_retries + 1):
            try:
                ...
                if response.status_code in _RETRYABLE_STATUS_CODES:
                    last_exception = AxiomBaseAPIError(
                ...
                last_exception = AxiomBaseConnectionError(
                    f"Failed to connect to AxiomBase at {self._base_url}: {exc}"
                )
                if attempt < self._max_retries:
                    delay = _compute_backoff_delay(attempt)
                    logger.warning(
                ...
                last_exception = AxiomBaseTimeoutError(
        ...
        raise AxiomBaseAPIError(  # pragma: no cover
    ...
    async def assert_fact(
        ...
        Facts are the fundamental units of knowledge in AxiomBase. A fact
        ...
        Raises:
            AxiomBaseValidationError: If the fact request is malformed.
            AxiomBaseConflictError: If the fact contradicts existing knowledge.
    ...
    async def assert_rule(
        ...
        Raises:
            AxiomBaseValidationError: If the rule request is malformed.
    ...
    async def __aenter__(self) -> AxiomBaseClient:
...
class SyncAxiomBaseClient:
    """Synchronous wrapper around :class:`AxiomBaseClient`.
    ...
    Args:
        base_url: The base URL of the AxiomBase server.
        api_key: Optional API key for authentication.
    ...
    def __init__(
        ...
        self._async_client = AxiomBaseClient(
            base_url=base_url,
            api_key=api_key,
            database=database,
            tenant_id=tenant_id,
    ...
    def assert_fact(
        ...
        """Assert a fact into the knowledge base. See :meth:`AxiomBaseClient.assert_fact`."""
        return self._run(
    ...
    def assert_rule(
        ...
        """Assert a logical rule. See :meth:`AxiomBaseClient.assert_rule`."""
    ...
    def query(
        ...
        """Query facts matching a pattern. See :meth:`AxiomBaseClient.query`."""
    ...
    def infer(
        ...
        """Run inference. See :meth:`AxiomBaseClient.infer`."""
        return self._run(
    ...
    def retract(
        ...
        """Retract a fact. See :meth:`AxiomBaseClient.retract`."""
        return self._run(
    ...
    def context_window(
        ...
        """Get context window. See :meth:`AxiomBaseClient.context_window`."""
        return self._run(
    ...
    def temporal_query(
        ...
        """Temporal query. See :meth:`AxiomBaseClient.temporal_query`."""
        return self._run(
    ...
    def consolidate(self) -> ConsolidationResult:
        """Run memory consolidation. See :meth:`AxiomBaseClient.consolidate`."""
        return self._run(self._async_client.consolidate())

    def decay(self, threshold: float | None = None) -> DecayResult:
        """Run memory decay. See :meth:`AxiomBaseClient.decay`."""
    ...
    def set_priority(
        ...
        """Set fact priority. See :meth:`AxiomBaseClient.set_priority`."""
        return self._run(
    ...
    def execute(self, command: str) -> str:
        """Execute a Logiql DSL command. See :meth:`AxiomBaseClient.execute`."""
    ...
    def __enter__(self) -> SyncAxiomBaseClient:
...
def _raise_for_status(response: httpx.Response) -> None:
    ...
    if status == 400:
        raise AxiomBaseValidationError(message, details=details)
    elif status == 404:
        raise AxiomBaseNotFoundError(message)
    elif status == 409:
        raise AxiomBaseConflictError(message)
    else:
        raise AxiomBaseAPIError(
            message=message,
            status_code=status,
