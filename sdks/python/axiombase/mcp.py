...
Wraps the AxiomBase server's ``POST /mcp`` endpoint, which speaks the
JSON-RPC 2.0 based MCP protocol.  This allows using AxiomBase as an
MCP tool provider in any MCP-compatible AI agent framework.

Usage::

    from axiombase.mcp import AxiomBaseMCPClient

    async with AxiomBaseMCPClient("http://localhost:9300") as mcp:
...
logger = logging.getLogger("axiombase.mcp")
...
class MCPError(AxiomBaseAPIError):
...
class AxiomBaseMCPClient:
    """MCP client for the AxiomBase server.

    Communicates with the AxiomBase server's ``POST /mcp`` endpoint using
    ...
    async def _jsonrpc(
        ...
        Raises:
            ...
            AxiomBaseConnectionError: If the server is unreachable.
            AxiomBaseTimeoutError: If the request times out.
        """
        ...
        try:
            ...
            raise AxiomBaseConnectionError(
                f"Failed to connect to AxiomBase MCP endpoint at {self._base_url}/mcp: {exc}"
            ) from exc
        except httpx.TimeoutException as exc:
            raise AxiomBaseTimeoutError(
    ...
    async def list_tools(self) -> list[MCPToolDefinition]:
        """List all tools available on the AxiomBase MCP server.
    ...
    async def call_tool(
        ...
        """Call a tool on the AxiomBase MCP server.
    ...
    async def __aenter__(self) -> AxiomBaseMCPClient:
